...
def getLoosingScore(calledNumbers, boards):
	for number in calledNumbers:
		newBoards = []
		for board in markBoards(number, boards):
			if not isBoardWinner(board):
				newBoards.append(board)
			if len(boards) == 1 and len(newBoards) == 0:
				unmarkedSum = sum([sum(line) for line in boards[0]])
				return unmarkedSum * number
		boards = newBoards
	return 'The givens have changed, son'
...
def solution():
	...
	print ('Part 1:\n' + str(getWinningScore(calledNumbers, boards[:])) + '\n')
	print ('Part 2:\n' + str(getLoosingScore(calledNumbers, boards[:])))

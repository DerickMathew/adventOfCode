def findValue(instructions, wireName, overridenWire, overwrittenValue):
  wireValues = {}
  ops = {}
  opLookUp = {
    'set': 0,
  ...
  if overridenWire in ops: del ops[overridenWire]
  wireValues[overridenWire] = overwrittenValue
  while len(ops):
    opsToDelete = []
    for op in ops:
...
def solution(reportLines):
  ...
  print(findValue(instructions, 'a', 'b', 16076))
